...
rcl_publisher_t publishero_debug;
...
void callback_cmd_vel(const void * msgin_cmd_vel)
{
    ...
    RCSOFTCHECK(rcl_publish(&publisher, &fmsg_cmd_vel, NULL));
    ...
    float pwm_r = cross_product(rad_per_sec_r, RAD_SEC_MAX, FREQ_MAX);
    float pwm_l = cross_product(rad_per_sec_l, RAD_SEC_MAX, FREQ_MAX);

    if(rad_per_sec_r > 0.0) {
        gpio_set_level(DIR_PIN_RIGHT, 1);
    } else {
        gpio_set_level(DIR_PIN_RIGHT, 0);
    }
    
    if(rad_per_sec_l > 0.0) {
        gpio_set_level(DIR_PIN_LEFT, 1);
    } else {
        gpio_set_level(DIR_PIN_LEFT, 0);
    }

    mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM0A, 50.0f);
    mcpwm_set_frequency(MCPWM_UNIT_0, MCPWM_TIMER_0, (int)pwm_r);

    // mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM0B, pwm_l);
}
...
void initPins(){
    ...
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM0B, PIN_PWM_LEFT);
    ...
    mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM0A, 99.0f);
    mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM0B, 50.0f);
}
